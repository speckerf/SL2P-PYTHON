...
def wrapperNNets(network, netOptions, imageInput, mode: str):
    assert mode in ["estimation", "uncertainty"]
    netList = network[netOptions["variable"] - 1]

    # save to the network parameters to json file:
    folder = "felix-params"
    filename = f"{folder}/{netOptions["Name"]}-{mode}_SL2P_Corrected_LeafToolBox.json"

    net_params = network[netOptions["variable"] - 1][0][0]

    inp_slope = net_params["inpSlope"]
    inp_offset = net_params["inpOffset"]
    h1wt = net_params["h1wt"]
    h1bi = net_params["h1bi"]
    h2wt = net_params["h2wt"]
    h2bi = net_params["h2bi"]
    out_slope = net_params["outSlope"]
    out_bias = net_params["outBias"]
