...
from tools import dictionariesSL2P, read_sentinel2_safe_image, toolsNets
...
def SL2P(sl2p_inp, variableName, imageCollectionName, outPath=None):
    ...
    estimate = toolsNets.wrapperNNets(SL2P, netOptions, sl2p_inp, mode="estimation")
    uncertainty = toolsNets.wrapperNNets(
        errorsSL2P, netOptions, sl2p_inp, mode="uncertainty"
    )
...
def prepare_sl2p_inp(s2, variableName, imageCollectionName):
    ...
    s2["SZA"] = read_sentinel2_safe_image.resample_image(
        s2["SZA"], factor, interpolation="bilinear"
    )
    s2["SAA"] = read_sentinel2_safe_image.resample_image(
        s2["SAA"], factor, interpolation="bilinear"
    )
    s2["VZA"] = read_sentinel2_safe_image.resample_image(
        s2["VZA"], factor, interpolation="bilinear"
    )
    s2["VAA"] = read_sentinel2_safe_image.resample_image(
        s2["VAA"], factor, interpolation="bilinear"
    )
    ...
    for band_id, band in enumerate(netOptions["inputBands"]):
        sl2p_inp.update(
            {
                band: (s2[band] + netOptions["inputOffset"][band_id])
